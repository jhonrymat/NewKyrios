<div class="modal fade" id="modal-finalizar-{{ $orden->codigo }}" tabindex="-1" role="dialog"
    aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <form id="finalizarForm-{{ $orden->codigo }}">
                @csrf
                @method('PUT')
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title">Finalizar Orden N° {{ $orden->codigo }}</h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="equipo" class="form-control" value="{{ $orden->equipo }}" required readonly>
                    </div>

                    <div class="form-group">
                        <label for="marca">Marca</label>
                        <input type="text" name="marca" class="form-control" value="{{ $orden->marca }}" required readonly>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="otros" class="form-control" value="{{ $orden->otros }}" required readonly>
                    </div>

                    <div class="form-group">
                        <label for="notacliente">Nota del Cliente</label>
                        <textarea name="notacliente" class="form-control" rows="4" readonly>{{ $orden->notacliente }}</textarea>
                    </div>

                    <div class="form-group">
                        <label for="observaciones">Observaciones</label>
                        <textarea name="observaciones" class="form-control" rows="4" readonly>{{ $orden->observaciones }}</textarea>
                    </div>

                    <div class="form-group">
                        <label for="valor">Valor Final</label>
                        <input type="text" name="valor" class="form-control" value="{{ $orden->valor }}" required>
                    </div>

                    <div class="form-group">
                        <label for="notatecnico">Nota del tecnico</label>
                        <textarea name="notatecnico" class="form-control" rows="4" required>{{ $orden->notatecnico }}</textarea>
                    </div>
                </div>
